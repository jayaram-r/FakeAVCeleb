...
class CustumDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.data_video:
            # TODO: should this be `self.data_video[idx]`?
...
class Ensemble(nn.Module):
    ...
    def forward(self, frame, video):
        ...
        # TODO: is this the average of logits or predicted probabilities?
        out = feat1 + feat2
        out = out / 2
        return out
...
MODELS_NAME = 'Meso4'
# checkpoints for model loaders :  [VIDEO(A&B), FRAME(A&C)]
...
ecls = Ensemble(models=models, device=device)
...
for i, data in enumerate(test_iterator):
    with torch.no_grad():
        ...
        integer_encoded_2 = integer_encoded_2.reshape(
            len(integer_encoded_2), 1)
        ...
        _y_true = np.array(torch.zeros(_y_true.shape).scatter(
            1, _y_true_argmax.unsqueeze(1), 1), dtype=np.int8)
        y_true = np.concatenate((y_true, _y_true))
        a = _y_pred.argmax(1)
        _y_pred = np.array(torch.zeros(_y_pred.shape).scatter(
            1, a.unsqueeze(1), 1), dtype=np.int8)
